...
class IouLoss(nn.Module):
    ...
    def forward(self, loc_p, loc_t,prior_data):
        ...
        elif self.loss == 'Focal-iou':
            iou = bbox_overlaps_iou(decoded_boxes,loc_t)
            confidence = iou
            loss = 2.477 * torch.pow((1 - 0.5 * confidence), 2) * (1 - iou)
            loss = torch.sum(loss)
        elif self.loss == 'Focal-ciou':
            ciou = bbox_overlaps_ciou(decoded_boxes,loc_t)
            confidence = (ciou + 2) / 3
            loss = 3.3 * torch.pow((1 - 0.5 * confidence), 2) * (1 - ciou)
            loss = torch.sum(loss)
        elif self.loss == 'Focal-diou':
            diou = bbox_overlaps_diou(decoded_boxes,loc_t)
            confidence = (diou + 1) / 2
