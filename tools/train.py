...
def train():
    ...
    #custom loss: Focal-iou, Focal-diou
    print(cfg['losstype'])
    ...
    if args.visdom:
        ...
        iter_plot = create_vis_plot(viz,'Iteration', 'Loss', vis_title, vis_legend, cfg)
        epoch_plot = create_vis_plot(viz,'Epoch', 'Loss', vis_title+" epoch loss", vis_legend, cfg)
    ...
    for epoch in range(args.max_epoch):
            ...
            if args.visdom:
                if iteration>20 and  iteration% 10 == 0:
                    update_vis_plot(viz,iteration, loss_l.item(), loss_c.item(),
                                iter_plot, epoch_plot, 'append', work_name=cfg['work_name'])
        ...
        if args.visdom:
            update_vis_plot(viz, epoch, loc_loss, conf_loss, epoch_plot, epoch_plot,
                                'append', epoch_size, work_name=cfg['work_name'])
        loc_loss = 0
        conf_loss = 0
...
def create_vis_plot(viz,_xlabel, _ylabel, _title, _legend, cfg):
    return viz.line(
        X=torch.zeros((1,)).cpu(),
...
def update_vis_plot(viz,iteration, loc, conf, window1, window2, update_type,
    ...
    # save the localization loss to file for further comparasion
    loss_file = args.work_dir+ work_name + "/loc_loss_" + args.work_dir[:-1] + ".xlsx"
    if not os.path.exists(loss_file):
        os.mkdir(args.work_dir+ work_name)
        print(loss_file)
        workbook = xlsxwriter.Workbook(loss_file)
        worksheet = workbook.add_worksheet('s001') 
        workbook.close()


    book = load_workbook(loss_file)
    writer = pd.ExcelWriter(loss_file, engine='openpyxl')
    writer.book = book
    writer.sheets = {ws.title: ws for ws in book.worksheets}

    append_data = {'loc_loss': loc, 'clc_loss': conf, 'loc/all': 5 * loc / (5 * loc + conf)}

    print(append_data)
    df1 = pd.DataFrame(append_data, index=[0])
