...
def do_python_eval(output_dir, set_type,thresh, use_07=True):
    cachedir = os.path.join(output_dir, 'annotations_cache')
    imgsetpath = os.path.join(output_dir,'ImageSets',
                          'Main', '{:s}.txt')
    annopath = os.path.join(output_dir, 'Annotations', '%s.xml')
    ...
    for i, cls in enumerate(labelmap):
        ...
        rec, prec, ap = voc_eval(
           ...
           ovthresh=thresh, use_07_metric=use_07_metric)
        aps += [ap]
...
def evaluate_detections(data_dir,box_list,dataset, thresh, eval_type = 'test'):
    #write the det result to dir
    write_voc_results_file(data_dir,box_list, dataset, eval_type)
    return do_python_eval(data_dir,eval_type,thresh=thresh)
...
if __name__ == '__main__':
    ...
    for thresh in np.arange(0.5,1,0.05):
        result = evaluate_detections(devkit_path,all_boxes, dataset,thresh, 'test')
        results.append(result)
    print(results[0], results[5], sum(results)/10)
